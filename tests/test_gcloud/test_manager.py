...
import subprocess
from src.gcloud.manager import GCloudManager, GCloudProject, QuotaStatus
...
def test_get_current_project_none(manager):
    with patch("subprocess.run", side_effect=subprocess.CalledProcessError(1, ["gcloud"])):
        project = manager.get_current_project()
        ...
        with patch("subprocess.run"):
            ...
            mock_run_cmd.side_effect = [
                ...
                [ # list projects output
                    {"projectId": "auto-gen-id", "name": "SOS Agent", "projectNumber": "1", "lifecycleState": "ACTIVE"}
                ]
            ]

            project = manager.create_project(project_id="auto-gen-id", auto_confirm=True)
...
def test_check_quota_status_failed(manager):
    with patch.object(manager, "_run_gcloud_command", side_effect=Exception("API Error")):
        status = manager.check_quota_status("proj")
